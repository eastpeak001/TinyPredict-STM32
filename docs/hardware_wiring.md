...
V0.4B PCB 使用三脚有源蜂鸣器模块作为报警输出，模块为低电平触发。
...
固件启动后默认 PB12 输出高电平关闭蜂鸣器；NORMAL 和 CALIBRATING 状态下蜂鸣器关闭，WARNING 状态短鸣提示，ALARM 状态下蜂鸣器报警已完成实物测试。
## 供电注意事项
...
- MPU6050 必须固定在被测物体上，线束也要固定，避免 I2C 接线晃动导致 OLED 显示异常。
